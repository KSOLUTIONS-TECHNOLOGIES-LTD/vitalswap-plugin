...
class WC_Gateway_Custom_VitalSwap_Blocks_Support extends AbstractPaymentMethodType {

	/**
	 * Initializes the payment method type.
	 */
	public function initialize() {
		$vitalswap_gateway_settings        = get_option( 'woocommerce_vitalswap_settings', array() );
		$custom_vitalswap_gateway_settings = get_option( "woocommerce_{$this->name}_settings", array() );

		$this->settings = wp_parse_args( $custom_vitalswap_gateway_settings, $vitalswap_gateway_settings );
	...
	public function get_payment_method_script_handles() {
		$script_asset_path = plugins_url( "/assets/js/blocks/frontend/blocks/{$this->name}.asset.php", WC_VitalSwap_MAIN_FILE );
		$script_asset      = file_exists( $script_asset_path )
			? require $script_asset_path
			: array(
				'dependencies' => array(),
				'version'      => WC_VitalSwap_VERSION,
			);

		$script_url = plugins_url( "/assets/js/blocks/frontend/blocks/{$this->name}.js", WC_VitalSwap_MAIN_FILE );
	...
	public function get_payment_method_data() {
		...
		if ( empty( $payment_icons ) ) {
			$payment_icons = array( 'vitalswap' );
		}

		$payment_icons_url = array();
		foreach ( $payment_icons as $payment_icon ) {
			$payment_icons_url[] = WC_HTTPS::force_https_url( plugins_url( "assets/images/{$payment_icon}.png", WC_VitalSwap_MAIN_FILE ) );
		}
	...
	public function failed_payment_notice( PaymentContext $context, PaymentResult &$result ) {
		if ( $this->name === $context->payment_method ) {
			add_action(
				'wc_gateway_vitalswap_process_payment_error',
