...
class WC_Gateway_VitalSwap extends WC_Payment_Gateway_CC
{
	...
	public function __construct()
	{
		...
		$this->method_description = sprintf(__('VitalSwap provide merchants with the tools and services needed to accept online payments from local and international customers using Mastercard, Visa, Verve Cards and Bank Accounts. <a href="%1$s" target="_blank">Sign up</a> for a VitalSwap account, and <a href="%2$s" target="_blank">get your API keys</a>.', 'woo-vitalswap'), 'https://VitalSwap.com', 'https://dashboard.VitalSwap.com/#/settings/developer');
	...
	public function get_icon()
	{
		...
		if ('GH' === $base_location['country']) {
			$icon = '<img src="' . WC_HTTPS::force_https_url(plugins_url('assets/images/VitalSwap-gh.png', WC_VitalSwap_MAIN_FILE)) . '" alt="VitalSwap Payment Options" />';
		} elseif ('ZA' === $base_location['country']) {
			$icon = '<img src="' . WC_HTTPS::force_https_url(plugins_url('assets/images/VitalSwap-za.png', WC_VitalSwap_MAIN_FILE)) . '" alt="VitalSwap Payment Options" />';
		} elseif ('KE' === $base_location['country']) {
			$icon = '<img src="' . WC_HTTPS::force_https_url(plugins_url('assets/images/VitalSwap-ke.png', WC_VitalSwap_MAIN_FILE)) . '" alt="VitalSwap Payment Options" />';
		} elseif ('CI' === $base_location['country']) {
			$icon = '<img src="' . WC_HTTPS::force_https_url(plugins_url('assets/images/VitalSwap-civ.png', WC_VitalSwap_MAIN_FILE)) . '" alt="VitalSwap Payment Options" />';
		} else {
			$icon = '<img src="' . WC_HTTPS::force_https_url(plugins_url('assets/images/VitalSwap-wc.png', WC_VitalSwap_MAIN_FILE)) . '" alt="VitalSwap Payment Options" />';
		}
...
?>
		...
		<h4>
			<strong><?php printf(__('Optional: To avoid situations where bad network makes it impossible to verify transactions, set your webhook URL <a href="%1$s" target="_blank" rel="noopener noreferrer">here</a> to the URL below<span style="color: red"><pre><code>%2$s</code></pre></span>', 'woo-vitalswap'), 'https://dashboard.VitalSwap.co/#/settings/developer', WC()->api_request_url('Tbz_WC_VitalSwap_Webhook')); ?></strong>
		</h4>
	...
	public function init_form_fields()
	{
		...
		$form_fields = array(
			...
			'title'                            => array(
				...
				'default'     => __('Debit/Credit Cards', 'woo-vitalswap'),
				'desc_tip'    => true,
			),
			'description'                      => array(
				...
				'default'     => __('Make payment using your debit and credit cards', 'woo-vitalswap'),
				'desc_tip'    => true,
			),
			'testmode'                         => array(
			...
			'subaccount_code'                  => array(
				...
				'class'       => 'woocommerce_VitalSwap_subaccount_code',
				'default'     => '',
			),
			'split_payment_transaction_charge' => array(
				...
				'class'             => __('woocommerce_VitalSwap_split_payment_transaction_charge', 'woo-vitalswap'),
				'default'           => '',
			...
			'custom_gateways'                  => array(
				'title'       => __('Additional VitalSwap Gateways', 'woo-vitalswap'),
				'type'        => 'select',
				'description' => __('Create additional custom VitalSwap based gateways. This allows you to create additional VitalSwap gateways using custom filters. You can create a gateway that accepts only verve cards, a gateway that accepts only bank payment, a gateway that accepts a specific bank issued cards.', 'woo-vitalswap'),
				'default'     => '',
				'desc_tip'    => true,
				'options'     => array(
					''  => __('Select One', 'woo-vitalswap'),
					'1' => __('1 gateway', 'woo-vitalswap'),
					'2' => __('2 gateways', 'woo-vitalswap'),
					'3' => __('3 gateways', 'woo-vitalswap'),
					'4' => __('4 gateways', 'woo-vitalswap'),
					'5' => __('5 gateways', 'woo-vitalswap'),
				),
			),
			'saved_cards'                      => array(
				'title'       => __('Saved Cards', 'woo-vitalswap'),
				'label'       => __('Enable Payment via Saved Cards', 'woo-vitalswap'),
				'type'        => 'checkbox',
				'description' => __('If enabled, users will be able to pay with a saved card during checkout. Card details are saved on VitalSwap servers, not on your store.<br>Note that you need to have a valid SSL certificate installed.', 'woo-vitalswap'),
				'default'     => 'no',
				'desc_tip'    => true,
			),
	...
	public function process_redirect_payment_option($order_id)
	{
		...
		$VitalSwap_url = 'https://vitalswap.com/test/api_v2/vapiex/session';
		...
		$url = "https://vitalswap.com/test/api_v2/vapiex/session?user_identifier=$customer_email&transaction_type=checkout&transaction_amount=$amount&currency_iso=ngn&first_name=$order->get_billing_first_name()&last_name=$order->get_billing_last_name()&meta=$customer_email";
		...
		if (!is_wp_error($request) && (200 === wp_remote_retrieve_response_code($request) || 201 === wp_remote_retrieve_response_code($request))) {
			...
			return array(
				'result'   => 'success',
				//'redirect' => WC_HTTPS::force_https_url(plugins_url('vitalswap-checkout.php', WC_VitalSwap_MAIN_FILE))."?sId=$VitalSwap_response->session_id&isOTP=$VitalSwap_response->isOTP&email=$customer_email",
				'redirect' => "https://wp-plugin.webhostingng.org/vitalswap-checkout.php?sId=$VitalSwap_response->session_id&isOTP=$VitalSwap_response->isOTP&email=$customer_email",
	...
	public function verify_VitalSwap_transaction()
	{
		...
		if (isset($_REQUEST['VitalSwap_txnref'])) {
			$VitalSwap_txn_ref = sanitize_text_field($_REQUEST['VitalSwap_txnref']);
		} elseif (isset($_REQUEST['reference'])) {
			$VitalSwap_txn_ref = sanitize_text_field($_REQUEST['reference']);
		} else {
		...
		if ($VitalSwap_txn_ref) {
			...
			if (false !== $VitalSwap_response) {
				...
				if ('success' == $VitalSwap_response->data->status) {
					...
					$order_details = explode('_', $_REQUEST['VitalSwap_txnref']);
	...
	public function process_webhooks()
	{
		...
		if (!array_key_exists('HTTP_X_VitalSwap_SIGNATURE', $_SERVER) || (strtoupper($_SERVER['REQUEST_METHOD']) !== 'POST')) {
			exit;
		}
	...
	private function get_VitalSwap_transaction($VitalSwap_txn_ref)
	{
		...
		$VitalSwap_url = 'https://api.VitalSwap.co/transaction/verify/' . $VitalSwap_txn_ref;
